<?php
    ...
    * define module title
    */
    $module_title .= ' <span class="titleseperator">' . $GCMS['CONFIG']['title_separator'] . '</span> ' . $GCMS['LANG']['NEWS']['index'];
    ...
    */
    $GCMS['TEMPLATE']->set_filenames(array(
        'news_index' => 'news/index.tpl',
        'news_work' => 'news/work.tpl',
        'news_comments' => 'news/comments.tpl',
        'news_pagination' => 'base/pagination.tpl',
        'post_form' => 'post/form.tpl',
    ));
	...
	if($total_news) {
		...
		*/
		for($i=1; $row = mysql_fetch_assoc($newsselect); $i++) {
			...
			*/
			$newscontent = strip_tags(gcms_changetext($row['content']));
			...
			* set news blockvar
			*/
			$GCMS['TEMPLATE']->assign_block_vars('newsposts', array(
				'SECTION' => $row['section'],
				'TITLE' => $row['title'],
		...
		*/
		$GCMS['TEMPLATE']->assign_var_from_handle('PAGINATION_CONTENT', 'news_pagination');


		/**
		* parse news index template
		*/
		$GCMS['TEMPLATE']->pparse('news_index');
	} else {
		/**                                                                 
		* assign message vars       
		*/
